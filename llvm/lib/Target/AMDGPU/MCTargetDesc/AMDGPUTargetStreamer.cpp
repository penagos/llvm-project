...
void AMDGPUTargetELFStreamer::emitAMDGPUInfo(
  ...
  // ELF SHT_STRTAB convention: offset 0 is reserved for the empty string, so
  // the pool starts with a leading null byte and real strings land at
  // offset >= 1. Consumers like `llvm-readobj --string-dump` rely on this.
  StrPool.push_back('\0');
